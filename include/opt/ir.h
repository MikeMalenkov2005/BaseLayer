...
#define IR_VALUE_INDEX_MASK ((U32)(IR_INVALID_INDEX >> 1))
#define IR_VALUE_ARRAY_BIT  (~IR_VALUE_INDEX_MASK)

typedef enum IR_OpCode
{
  IR_INVL, /* INVALID OPERATION */

  IR_LDFA, /* LOAD FUNCTION ADDRESS                       ( -> PTR)           */
  IR_LDIV, /* LOAD IMMEDIATE VALUE                        ( -> VAL)           */
...
{
  U32 name;
  U32 value;  /* IR_INVALID_INDEX FOR EXTERNAL            */
  UZ size;    /* TOTAL SIZE INCLUDING UNINITIALIZED PARTS */
} IR_Global;
...
{
  U32 name;           /* IR_INVALID_INDEX FOR ANONYMUS  */
  U32 args;           /* MAX_U32 FOR VARIADIC           */
  DS_Array(IRI) body; /* NULL FOR EXTERNAL              */
} IR_Function;
...
typedef struct IR
{
  ...
  DS_Vector(STR) arrays;
  DS_Vector(U64) values;
} IR;
